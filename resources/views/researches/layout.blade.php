<x-layout>
    ...
    <main class="d-flex flex-column justify-content-center w-100 bg-body-secondary">
        ...
        <div class="container d-flex flex-column pb-5 row-gap-4">
            <section class="w-100">
                <a href="/collections/researches#researches-form" class="btn btn-outline-success">
                    New Research
                            ...
                            <tr>
                                <td style="min-width: 140px;">
                                    <form id="delete-research-{{ $research->id }}"
                                        action="/collections/researches/{{ $research->id }}" method="POST" class="d-none">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit">DELETE</button>
                                    </form>
                                    <a title="Copy" href="/collections/researches/{{ $research->id }}/copy#researches-form" class="btn btn-light btn-sm">
                                        <i class="bi bi-copy"></i>
                                    </a>
                                    <a title="Edit" href="/collections/researches/{{ $research->id }}/edit#researches-form" class="btn btn-light btn-sm">
                                ...
                                <td class="text-capitalize">{{ $research->type }}</td>
                                <td>{{ $research->title }}</td>
                                <td class="text-end">{{ $research->year_submitted }}</td>
            ...
            <section class="d-block">
                <div id="researches-form" class="card p-3 w-full shadow">
                    <div class="card-body">
                        {{ $form ?? '' }}
                    </div>
                </div>
