    ...
    @slot('breadcrumb')
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0 border py-2 px-3 bg-white rounded">
                ...
                <li class="breadcrumb-item"><a href="/settings/campuses">Campuses</a></li>
                <li class="breadcrumb-item active" aria-current="page">{{ $selected->code }}</li>
            </ol>
        </nav>
    @endslot
    @slot('form')
        <form action="/settings/campuses/{{ $selected->id }}" method="POST">
            @csrf
            @method('PATCH')

            <h4 class="text-body-secondary">Edit this campus</h4>
            <hr>
            <div class="mb-2">
                @php
                    if($errors->has('code')) {
            ...
            <div class="d-flex gap-2">
                ...
                <a href="/settings/campuses" class="w-100 btn btn-outline-secondary px-3">Cancel</a>
            </div>
        </form>
    @endslot
@endcomponent
